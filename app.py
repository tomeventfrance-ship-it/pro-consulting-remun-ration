...
def shared_payment_preferences_scope():
    """Référence collective des modes définis par FONDATEUR ADMIN."""
    return "shared:payment_preferences"
...
def load_shared_payment_preferences(database_url):
    """Relit brièvement la référence collective sans ralentir les écrans."""
    return load_persistent_scope(database_url, shared_payment_preferences_scope())


def persist_payment_preferences(database_url, user_email, share_globally=False):
    """Enregistre la copie nettoyée et, pour le fondateur, la partage."""
    ...
    scopes = {payment_preferences_scope(user_email): preferences}
    if share_globally:
        scopes[shared_payment_preferences_scope()] = preferences
    save_persistent_scopes(database_url, scopes, user_email)
    confirmation_scope = (
        shared_payment_preferences_scope()
        if share_globally
        else payment_preferences_scope(user_email)
    )
    confirmed = clean_payment_preferences(
        load_persistent_scope(database_url, confirmation_scope)
    )
    if confirmed.get("saved_at") != preferences["saved_at"]:
        raise RuntimeError("La base n'a pas confirmé les modes de paiement.")
    if share_globally:
        load_shared_payment_preferences.clear()
...
payment_preferences_first_load = (
    st.session_state.get("payment_preferences_loaded_for")
    != current_user_email
)
if persistent_settings_available:
    try:
        shared_payment_preferences = clean_payment_preferences(
            load_shared_payment_preferences(database_url)
        )
        shared_preferences_exist = bool(
            shared_payment_preferences.get("saved_at")
        )
        current_preferences = clean_payment_preferences(
            st.session_state.get("payment_preferences", {})
        )
        if shared_preferences_exist and (
            payment_preferences_first_load
            or shared_payment_preferences.get("saved_at")
            != current_preferences.get("saved_at")
        ):
            st.session_state.payment_preferences = shared_payment_preferences
            for payment_widget_key in (
                "creator_payment_editor",
                "consultant_payment_editor",
                "responsable_payment_editor",
                "admin_director_payment_editor",
                "director_payment_mode_selector",
            ):
                st.session_state.pop(payment_widget_key, None)
    except Exception:
        shared_preferences_exist = False
        if payment_preferences_first_load:
            st.sidebar.warning(
                "Les modes de paiement partagés n’ont pas pu être chargés "
                "pour le moment."
            )
else:
    shared_preferences_exist = False
...
if payment_preferences_first_load:
    if not shared_preferences_exist:
...
if page == "🏠 Accueil":
    ...
    edited_payment_table = st.data_editor(
        ...
        disabled=[
            ...
            "Rémunération 💎",
            "Montant facture (€)",
        ] + (["Mode paiement"] if current_user_role != "admin" else []),
        column_config={
            "Inclure rémunération créateur": (
    ...
    if st.button(
        ...
        try:
            persist_payment_preferences(
                database_url, current_user_email, share_globally=True
            )
            st.success(
    ...
    if current_user_role != "admin":
        st.caption(
            "🔒 Modes de paiement définis par FONDATEUR ADMIN et "
    ...
    saved_director_payment_mode = st.session_state.payment_preferences.get(
        ...
        payment_entity_key(current_user_name),
        st.session_state.payment_preferences.get("director_mode", "Diamants"),
    )
    if (
        st.session_state.get("director_payment_mode_selector")
        != saved_director_payment_mode
    ):
        st.session_state.director_payment_mode_selector = (
            saved_director_payment_mode
        )
    ...
    st.caption(
        "🔒 Mode de paiement défini par FONDATEUR ADMIN et synchronisé "
        "automatiquement."
    )
